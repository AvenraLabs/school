...
export default function DashboardPage() {
  ...
  const [closedPosters, setClosedPosters] = useState(() => {
    try {
      return JSON.parse(sessionStorage.getItem("closed_posters") || "[]");
    } catch {
      return [];
    }
  });

  const handleClosePoster = (posterId) => {
    const updated = [...closedPosters, posterId];
    setClosedPosters(updated);
    sessionStorage.setItem("closed_posters", JSON.stringify(updated));
  };
  ...
  const renderPosters = () => {
    const activePosters = posters.filter(p => !closedPosters.includes(p.id));
    if (!activePosters || activePosters.length === 0) return null;
    return (
      <Stack spacing={2} sx={{ mb: 3 }}>
        {activePosters.map((poster) => (
          ...
          >
            <IconButton
              size="small"
              onClick={() => handleClosePoster(poster.id)}
              sx={{
                position: "absolute",
                top: 8,
                right: 8,
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center" sx={{ pr: 3 }}>
              {poster.image_url && (
                <Box
                  sx={{
                    width: { xs: "100%", sm: 120 },
