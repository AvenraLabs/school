...
- `POST /api/ai/videos` — Create background Google Veo video generation job.
- `GET /api/ai/videos/stream/:id` — Public video streaming endpoint (HTTP 206 byte-range streaming proxy from GCS).
- `GET /api/ai/videos/teacher/my-videos` — List videos generated by teacher.
- `GET /api/ai/videos/student/class-videos` — List class videos available for student.
- `GET /api/ai/videos/:id` — Poll status of video job.
