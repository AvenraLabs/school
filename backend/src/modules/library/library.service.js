...
export const returnBookService = async (issueId, school_id, returned_by, { status, fine_amount, remarks }) => {
  ...
  try {
    ...
    // If fine > 0 and student issue, check library_fine_to_fees toggle
    if (issue.student_id && fine_amount && fine_amount > 0) {
      const school = await School.findByPk(school_id, {
        attributes: ["library_fine_to_fees"],
        transaction: t,
      });

      if (school?.library_fine_to_fees) {
        await createLibraryFineEntry(school_id, issue.student_id, fine_amount, issue.id, t);
      }
    }
...
/** Auto-create a "Library Fine" term ledger entry for the student to pay in Fee Management */
const createLibraryFineEntry = async (school_id, student_id, amount, issueId, transaction) => {
  try {
    const fineAmount = parseFloat(amount);
    if (!fineAmount || fineAmount <= 0) return;

    // 1. Find or create the "Library Fine" fee category
    let [category] = await FeeCategory.findOrCreate({
      where: { school_id, name: "Library Fine" },
      defaults: { school_id, name: "Library Fine", is_active: true, is_archived: false },
      transaction,
    });

    if (category.is_archived) {
      await category.update({ is_archived: false, is_active: true }, { transaction });
    }

    const { default: StudentFeeLedger } = await import("../fees/student-fee-ledger.model.js");
    const { default: StudentTermLedger } = await import("../fees/student-term-ledger.model.js");
    const AcademicYear = (await import("../academic-years/academic-year.model.js")).default;
    ...
    if (!currentYear) {
      console.warn("[Library] No current academic year found — skipping fee entry creation");
      return;
    }

    // 2. Find or create master student fee ledger
    let ledger = await StudentFeeLedger.findOne({
      where: { school_id, student_id, academic_year_id: currentYear.id },
      transaction,
    });
    ...
    // 3. Find or create a pending StudentTermLedger for "Library Fine"
    const todayStr = new Date().toISOString().split("T")[0];

    let termLedger = await StudentTermLedger.findOne({
      where: {
        school_id,
        student_id,
        academic_year_id: currentYear.id,
        term_name: "Library Fine",
        status: { [Op.ne]: "paid" },
      },
      transaction,
    });
    ...
    if (termLedger) {
      const newTotal = Number(termLedger.total) + fineAmount;
      const newBalance = Number(termLedger.balance) + fineAmount;
      await termLedger.update(
        {
          total: newTotal,
          balance: newBalance,
          status: "pending",
      ...
      termLedger = await StudentTermLedger.create(
        {
          school_id,
          academic_year_id: currentYear.id,
          student_id,
    ...
    // 4. Recalculate and update master ledger
    const newGross = Number(ledger.gross_total) + fineAmount;
    const newNet = Number(ledger.net_total) + fineAmount;
    const newBalance = Number(ledger.balance) + fineAmount;
    ...
    console.error("[Library] Failed to create fee entry for library fine:", err.message);
  }
};
