...
3. A single school record is seeded for the deployment; super admin does not create schools through the app.

4. Super admin creates or updates the school admin user directly using backend setup or a dedicated admin user flow.

5. Activate the school admin user if needed by updating their account status through the admin user management flow.
...
Bulk: `POST /api/bulk/bulk-create`
