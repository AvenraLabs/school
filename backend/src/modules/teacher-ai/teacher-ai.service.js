...
export async function runTeacherAI({ user, aiType, payload }) {
  ...
  if (ragQuery && hasScope) {
    try {
      ...
      console.error("ChromaDB/RAG context retrieval failed, falling back to direct Gemini query:", err);
      chromaFailed = true;
    }
  }
...
export async function getTeacherAiHistoryService(userId) {
  return await AiChatLog.findAll({
    where: {
      user_id: userId,
      ai_type: {
        [Op.in]: ["question_paper", "lesson_summary"],
      },
    },
